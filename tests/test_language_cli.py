...
def test_language_select_predefined_with_heading_translation():
    ...
    with (
        ...
        patch("kittylog.language_cli.KITTYLOG_ENV_PATH", Path(tmpdir) / ".kittylog.env"),
...
def test_language_select_english_removes_setting():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with (
            patch("kittylog.language_cli.KITTYLOG_ENV_PATH", fake_path),
            patch("questionary.select") as mock_select,
            patch("kittylog.language_cli.unset_key") as mock_unset,
        ):
...
def test_language_selection_cancelled():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("kittylog.language_cli.KITTYLOG_ENV_PATH", fake_path), patch("questionary.select") as mock_select:
