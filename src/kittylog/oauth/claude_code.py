...
Adapted from gac's OAuth implementation for kittylog.
"""
...
def get_token_storage_path() -> Path:
    ...
    return Path.home() / ".kittylog.env"


def load_stored_token() -> str | None:
    """Load stored access token from .kittylog.env."""
...
def prompt_for_reauth() -> bool:
    """Prompt user to re-authenticate when token is expired or invalid.
    ...
    try:
        ...
        if not sys.stdin.isatty():
            logger.warning("Non-interactive terminal detected; cannot prompt for re-authentication")
            return False

        print("\n⚠️  Your Claude Code access token has expired or is invalid.")
        print("   Would you like to re-authenticate now?")
        print("   (You can also run 'kittylog config reauth' later)")

        response = input("\n   Re-authenticate now? [y/N]: ").strip().lower()

        if response in ("y", "yes"):
            print("\n🔐 Starting Claude Code re-authentication...")
            return authenticate_and_save(quiet=False)
        else:
            print("   Skipping re-authentication. Run 'kittylog config reauth' when ready.")
            return False
