...
from clog.git import get_all_tags, get_latest_tag, get_tags_since_last_changelog
...
def main_business_logic(
    ...
    if from_tag is None and to_tag is None:
        ...
        if not quiet:
            console.print(f"[cyan]Found {len(new_tags)} new tags to process: {', '.join(new_tags)}[/cyan]")
